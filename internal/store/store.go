...
func (sls SQLiteStore) GetAllItems(ordering string) ([]Item, error) {
	itemStmt := `
		select id, feedurl, guid, link, title, content, author, readat, favourite, publishedat, createdat, updatedat from items order by feedurl asc, coalesce(publishedat, createdat) %s;
	`
